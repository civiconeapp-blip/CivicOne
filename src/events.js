 ...
 * }
 *
 * Auto-sourced events (Phase 5 provenance pipeline) live in a SEPARATE
 * file, src/events-auto.js, generated by scripts/fetch-recpark-events.js —
 * never hand-edited, never merged into this file. Those events carry a
 * `source` field (naming the official feed) instead of `verified`; the
 * feed itself is the provenance under the Integrity Law. upcomingEvents()
 * below merges both files and accepts either verified OR source as valid
 * provenance — an event with neither never renders.
 */
...
/** Upcoming, provenance-verified events within the next `days` days.
 *  Merges hand-verified EVENTS with auto-sourced EVENTS_AUTO; an event
 *  needs either a `verified` stamp or a `source` field (plus a url) to
 *  ever render — never both missing. */
export function upcomingEvents(districtId = null, days = 60) {
  const now = new Date();
  now.setHours(0, 0, 0, 0);
  const cutoff = new Date(now);
  cutoff.setDate(cutoff.getDate() + days);
  return [...EVENTS, ...EVENTS_AUTO].filter((e) => {
    if (!(e.verified || e.source) || !e.url) return false;

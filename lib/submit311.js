...
export async function submitReport(opts) {
  ...
  try {
    ...
    if (wantsTracking) {
      ...
      // The panel this choice reveals can take longer than a flat sleep to
      // render — evidence from a real failure: the "Report Anonymously"
      // button (and every email field) had a 0x0 bounding box 1200ms after
      // the click, but "Report Anonymously" WAS present in the page's text
      // by the time the (unnecessary) email-fill attempt gave up ~40s
      // later. Poll for the button instead of guessing a fixed wait.
      const reportAnonBtn = page.locator('button:has-text("Report Anonymously")').first();
      const reportAnonVisible = await reportAnonBtn

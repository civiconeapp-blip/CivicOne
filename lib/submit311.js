    ...
    if (wantsTracking) {
      ...
      if (await reportAnonBtn.isVisible().catch(() => false)) {
        ...
        if (spec.anonymousNeedsEmail) {
          ...
          // Prior pages leave read-only/duplicate email-ish inputs (e.g. a
          // hidden profile-mapped field) in the DOM; :visible keeps a plain
          // lookup from matching them, and waitForFormElement's iframe
          // fallback covers forms that render this field in an embedded
          // widget instead of the main page.
          const email = await waitForFormElement(
            page,
            'input[type="email"]:visible, input[name*="email" i]:visible',
            ELEMENT_TIMEOUT

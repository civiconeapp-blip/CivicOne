...
export async function submitReport(opts) {
  ...
  try {
    ...
    if (wantsTracking) {
      const email = page.locator('input[type="email"]:visible, input[name*="email" i]:visible').last();
      ...
      if (await reportAnonBtn.isVisible().catch(() => false)) {
        ...
        if (spec.anonymousNeedsEmail) {
          ...
          // hidden profile-mapped field) in the DOM; :visible is required or
          // .last() can silently resolve to one that never becomes visible.
          const email = page.locator('input[type="email"]:visible, input[name*="email" i]:visible').last();
          await email.waitFor({ state: 'visible' });
          await email.fill(opts.email);
        }
